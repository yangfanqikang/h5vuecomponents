<template>
  <div v-if="width" class="uni-grid-item" :style="{width: width}">
    <div
      ...
      <div v-if="marker === 'dot'" :style="{ left: top + 'px', top: left + 'px' }" class="uni-grid-item__box-dot"/>
      <div v-if="marker === 'badge'" :style="{ left: top + 'px', top: left + 'px' }" class="uni-grid-item__box-badge">
        <uni-badge :text="text" :type="type" :size="size" :inverted="inverted"/>
      </div>
      <div v-if="marker === 'image'" :style="{ left: top + 'px', top: left + 'px' }" class="uni-grid-item__box-image">
        <img :style="{ width: imgWidth + 'px' }" :src="src" class="box-image" mode="widthFix"/>
      </div>
      <div class="uni-grid-item__box-item">
        <slot/>
      </div>
    </div>
  </div>
...
export default {
  ...
  // #ifdef H5
  mounted () {
    this.width = this.grid._getSize() + 'px'
...
<style scoped>
  ...
  div.widthStyle {
    width: calc(var(--width-style) - 10px);
    /*width: 224px;*/
    /*box-sizing: border-box;*/
  }
</style>
