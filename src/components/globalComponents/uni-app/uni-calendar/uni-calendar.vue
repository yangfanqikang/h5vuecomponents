<template>
  <div>
    <div v-if="maskShow && !insert" :class="{ 'ani-mask-show': aniMaskShow }" class="uni-calendar__mask"/>
    <div v-if="maskShow || insert" :class="{ 'ani-calendar-show': aniMaskShow, 'uni-calendar__static': insert }"
         class="uni-calendar__box">
      <div v-if="!insert" class="uni-calendar__nav">
        <div class="uni-calendar__nav-item" @click="close">取消</div>
        <div class="uni-calendar__nav-item" @click="confirm">确认</div>
      </div>
      <div class="uni-calendar__wrapper">
        <div class="uni-calenda__content">
          <div class="uni-calendar__panel">
            <div class="uni-calendar__date-befor" @click="dataBefor(-1, 'month')"><span
              class="iconfont icon-jiantou"></span></div>
            <div class="uni-calendar__panel-box">
              <div>{{ canlender.year }}年</div>
              <div>{{ canlender.month }}月</div>
            </div>
            <div class="uni-calendar__date-after uni-calendar__rollback" @click="dataBefor(1, 'month')"><span
              class="iconfont icon-jiantou "></span></div>
            <div class="uni-calendar__backtoday" @click="backtoday">回到今天</div>
          </div>
          <div v-if="isLunar" class="uni-calendar__day-detail">
            <div>{{ canlender.year + '年' + canlender.month + '月' + canlender.date + '日 （' + canlender.lunar.astro + ')'
              }}
            </div>
            <div>
